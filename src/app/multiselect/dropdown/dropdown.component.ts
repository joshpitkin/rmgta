...
@Component({
  selector: 'platform-multiselect-dropdown',
...
export class MultiselectDropdown
  ...
  checkAll() {
    if (!this.disabledSelection && this.model.length !== this.options.length) {
      this.model = [];
      this.addChecks(
  ...
  uncheckAll() {
    if (!this.disabledSelection && this.model.length > 0) {
            ...
            this.checkAllSearchRegister.forEach(searchTerm => {
              let filterOptions = this.applyFilters(
                this.options.filter(option =>
                  unCheckedOptions.indexOf(option.id) > -1
                ),
                searchTerm
              );
